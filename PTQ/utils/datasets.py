...
from utils.general import (DATASETS_DIR, LOGGER, NUM_THREADS, check_dataset, check_requirements, check_yaml, clean_str,
                           cv2, segments2boxes, xyn2xy, xywh2xyxy, xywhn2xyxy, xyxy2xywhn, xywh2xyxy_custom)
...
class LoadImagesAndLabels_custom() :
    ...
    def __init__(self, image_path, imgsz, label_path, stride=32, auto=True, resize=False):
        ...
        if '*' in p:
            pfiles = sorted(glob.glob(p, recursive=True))  # glob
        elif os.path.isdir(p):
            pfiles = sorted(glob.glob(os.path.join(p, '*.*')))  # dir
        elif os.path.isfile(p):
            pfiles = [p]  # files
        else:
            raise Exception(f'ERROR: {p} does not exist')
        ...
        if '*' in q:
            qfiles = sorted(glob.glob(q, recursive=True))  # glob
        elif os.path.isdir(q):
            qfiles = sorted(glob.glob(os.path.join(q, '*.*')))  # dir
        elif os.path.isfile(q):
            qfiles = [q]  # files
        else:
            raise Exception(f'ERROR: {q} does not exist')



        images = [x for x in pfiles if x.split('.')[-1].lower() in IMG_FORMATS]
        videos = [x for x in pfiles if x.split('.')[-1].lower() in VID_FORMATS]
        ni, nv = len(images), len(videos)
        ...
        if self.video_flag[self.count]:
            ...
            while not ret_val:
                ...
                if self.count == self.nf:  # last video
                    ...
                    image_path = self.files[self.count]
                    self.new_video(image_path)
                    ret_val, img0 = self.cap.read()

            self.frame += 1
            s = f'video {self.count + 1}/{self.nf} ({self.frame}/{self.frames}) {image_path}: '
            ...
            s = f'image {self.count}/{self.nf} {image_path}: '
            targets = []
